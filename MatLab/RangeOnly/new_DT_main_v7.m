...
theta_est0 = deg2rad(90); %l_ang + init_ang_err; %-pi+2*pi*rand(1);
...
%     noise = -mag + 2*mag*rand;
    ...
    theta_est(i) = theta_est(i-1) + T*u(:,i-1)'*w(:,i-1) + gamma*sign(u(:,i-1)'*w(:,i-1))*theta_update(i);
