...
from spawn_models import spawn_model
...
class Landmark:
    ...
    name: str = "landmark"
    path: str = "/home/alex/.gazebo/models/first_2015_trash_can/model.sdf"
    gain: float = 10.0
    ...
    def initialize(self, robot_x: float, robot_y: float) -> None:
        ...
        if self.simulation:
            ...
            # pose_.position.y = self._y_star
            # spawn_model(str(self.name + "_true"), self.path, pose_)
            pose = Pose()
            pose.position.x = self.x
            pose.position.y = self.y
            spawn_model(self.name, self.path, pose)
    ...
    def update_estimate(
        ...
        self.update_xy(robot_x, robot_y)
